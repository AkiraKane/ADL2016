...
tf.app.flags.DEFINE_string("train_history_file", "/tmp", "Train history file.")  # file name for perplexity csv
...
#tf.app.flags.DEFINE_boolean("use_attention", True,"In train the parameter is train_with_true_label.  In testing the model is use attention")
...
def train():
  ...
  with tf.Session(config=config) as sess:
    ...
    valid_perplexity_total_list = []
    valid_tagf1_total_list = [] 
    while model.global_step.eval() < FLAGS.max_training_steps:
      ...
      if current_step % FLAGS.steps_per_checkpoint == 0:
        ...
        valid_perplexity_total_list.append(valid_perplexity)
        valid_tagf1_total_list.append(valid_tagging_result['f1'])
        # record best results
        ...
        if valid_tagging_result['f1'] > best_valid_score:
          ...
          # save the best validation f1 model
          best_dir = os.path.join( FLAGS.train_dir , "best")
          if not os.path.exists(best_dir):
            os.makedirs(best_dir)
          checkpoint_path = os.path.join(best_dir , "best_model.ckpt")
          model.saver.save(sess, checkpoint_path, global_step=model.global_step)
    ...
    '''
    # save the perplexity and f1 change in csv
    check_num = int(FLAGS.steps_per_checkpoint)
    #p_file = FLAGS.test_output_file + "perplexity" + ".csv"  
    p_file = FLAGS.train_history_file + "_perplexity" + ".csv"  
    with open(p_file, 'w') as csvfile:
      p_writer = csv.writer(csvfile)
      for i in range(len(valid_perplexity_total_list)):
        p_writer.writerow(str(i*check_num), str(valid_perplexity_total_list[i]))
    #p_file = FLAGS.test_output_file + "f1" + ".csv"  
    p_file = FLAGS.train_history_file + "_f1" + ".csv"  
    with open(p_file, 'w') as csvfile:
      p_writer = csv.writer(csvfile)
      for i in range(len(valid_tagf1_total_list)):
        p_writer.writerow(str(i*check_num), str(valid_tagf1_total_list[i]))
    '''
def decode_file(test_file):
  ...
  vocab_path = FLAGS.data_dir + '/in_vocab_10000.txt'
  tag_vocab_path = FLAGS.data_dir + '/out_vocab_123.txt'
  ...
  lm_test_output_file = FLAGS.test_output_file + '_nouse_lm.txt'

  #intent_test_output_file = FLAGS.test_output_file + '.intent.hyp'
  intent_test_output_file = FLAGS.test_output_file + '_nouse.intent.txt'
  #intent_test_output_file = FLAGS.test_output_file 

  #tagging_test_output_file = FLAGS.test_output_file + '.tag.hyp'
  #tagging_test_output_file = FLAGS.test_output_file + '.tag.txt'
  tagging_test_output_file = FLAGS.test_output_file 
...
#                                             eval_sequence_length, bucket_id, True, use_attention=FLAGS.use_attention)
                _, step_loss, tagging_logits, classification_logits = model_test.joint_step(sess, eval_encoder_inputs, eval_encoder_inputs_shiftByOne, eval_lm_weights, eval_tags, eval_tag_weights, eval_labels, eval_intent_weights, 
                                             eval_sequence_length, bucket_id, True, train_with_true_label=True)
                ...
                # intent results
                hyp_label = np.argmax(classification_logits[0],0)
                #f_intent.write('%s' % rev_label_vocab[hyp_label])
                f_intent.write('%s' % rev_label_vocab[hyp_label].strip())
                f_intent.write('\n')
                f_intent.flush()
                # tagging results
                #f_tagging.write('%s' % ' '.join([rev_tag_vocab[np.argmax(x,1)] for x in tagging_logits[1:eval_sequence_length[0]]]))
                f_tagging.write('%s' % ' '.join([rev_tag_vocab[np.argmax(x,1)] for x in tagging_logits[1:eval_sequence_length[0]]]).strip())
                f_tagging.write('\n')
                f_tagging.flush()
